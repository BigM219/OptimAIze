...
class SuryaOCRBackend(BaseVLMBackend):
    """CPU backend for datalab-to/surya-ocr-2."""

    def __init__(
        self,
        model_id: str = "datalab-to/surya-ocr-2",
        device: str = "cpu",
        ...
        if auto_runtime != "off":
            profile = promoted_backend_profile("surya-ocr", model_id, config)
        ...
        self.model.eval()
        logger.info("Surya OCR 2 backend initialized on CPU")
    ...
    def _apply_cpu_optimizations(self) -> None:
        ...
        if self.quantize_mode == "lm_head":
            ...
            replaced = replace_linears_with_int8(
                ...
                skip_names=("self_attn", "linear_attn", "visual", "vision"),
            ...
            raise ValueError(f"Unsupported Surya OCR quantize_mode: {self.quantize_mode}")
        logger.info("Surya OCR CPU optimization quantize_mode=%s replaced=%d", self.quantize_mode, replaced)
    ...
    def generate_ocr(self, image: Image.Image, category: str = "plain") -> str:
        ...
        task_prompt = "Table Recognition:" if cat == "table" else "Text Recognition:"
        messages = [
            {
                "role": "user",
                "content": [
        ...
        tokenizer = getattr(self.processor, "tokenizer", None)
        non_table_cap = self.runtime_policy.non_table_max_new_tokens if self.runtime_policy else 64
